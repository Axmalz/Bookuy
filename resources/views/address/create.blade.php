...
<div class="w-full h-full bg-white flex flex-col relative">
    ...
    <!-- 2. Area Peta (Simulasi Interaktif) -->
    <div class="relative flex-grow overflow-hidden bg-gray-100 z-0">
        <!-- Gambar Peta (Bisa digeser-geser / Drag simulasi via CSS/JS sederhana atau statis) -->
        <!-- Kita buat statis penuh layar tapi bisa di-pan sedikit untuk efek -->
        <div id="map-container" class="w-[150%] h-[150%] -ml-[25%] -mt-[25%] bg-cover bg-center cursor-move"
             style="background-image: url('{{ asset('images/image-map.png') }}');">
        </div>

        <!-- Pin Kuning (Stuck di Tengah Layar) -->
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none z-10">
            <img src="{{ asset('images/icon-pin-map.png') }}" class="w-12 h-12 drop-shadow-lg animate-bounce">
        </div>

        <!-- Tombol "Set this Address" -->
        <div class="absolute bottom-8 left-6 right-6 z-20">
            <button id="open-form-btn" class="w-full bg-yellow-500 text-white font-bold text-lg py-3.5 rounded-full shadow-xl hover:bg-yellow-600 transition-transform active:scale-95">
                Set this Address
            </button>
        </div>
    </div>

    <!-- 3. Bottom Sheet Form (Hidden by default) -->
        ...
        <div id="sheet-content" class="absolute bottom-0 left-0 w-full bg-white rounded-t-[30px] transform translate-y-full transition-transform duration-300 flex flex-col shadow-2xl pointer-events-auto">
            ...
            <div class="px-6 pb-8 pt-2">
                <form id="address-form">
                    ...
                    <!-- Tombol Add (Default: Grey/Disabled style, JS will enable) -->
...
@push('scripts')
<script>
    // --- Map Interaction (Simple Drag) ---
    // (Opsional: Bisa ditambahkan JS drag-to-scroll sederhana jika diinginkan,
    //  tapi overflow-hidden + background image besar sudah cukup simulatif)
    ...
    const container = document.getElementById('address-sheet-container');
