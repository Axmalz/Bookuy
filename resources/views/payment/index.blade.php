...
<div class="w-full h-full bg-white flex flex-col relative">
    ...
    <!-- 2. Konten List -->
    <!-- DIEDIT: Mengubah px-6 menjadi px-4 agar segmen kartu lebih lebar -->
    <div class="flex-grow overflow-y-auto px-4 pt-6 pb-24 bg-white no-scrollbar relative z-0">
        ...
        <div class="space-y-4">
            ...
            <!-- Segmen Kartu -->
            <div class="w-full border rounded-2xl p-4 relative transition-all duration-300 shadow-sm flex flex-col justify-center
                        {{ $card->is_default ? 'border-blue-500 bg-blue-50/20' : 'border-gray-200 bg-white' }}">

                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3 flex-grow min-w-0">
                        <!-- Icon Visa/Mastercard (DIEDIT: Ukuran h-6 agar tidak terlalu besar) -->
                        ...
                        <!-- Tag Default -->
                        @if($card->is_default)
                            <span class="bg-gray-200 text-gray-600 text-[10px] font-bold px-2 py-0.5 rounded flex-shrink-0">Default</span>
                        @endif
                    </div>
                ...
                <!-- Tombol Edit & Delete (Hanya jika Default/Aktif) -->
                @if($card->is_default)
                <div class="flex items-center gap-4 mt-3 pl-9 animate-fade-in"> <!-- pl-9 agar sejajar teks (melewati lebar icon) -->
                    <a href="{{ route('payment.edit', $card->id) }}" class="text-blue-600 text-xs font-bold hover:underline transition-colors">
                        Edit
                    </a>
                    <form action="{{ route('payment.destroy', $card->id) }}" method="POST" onsubmit="return confirm('Hapus kartu ini?');" class="flex items-center">
                        @csrf @method('DELETE')
                        <button type="submit" class="text-red-500 text-xs font-bold hover:underline transition-colors">
                            Delete
                        </button>
                    </form>
                </div>
        ...
        @if($cards->count() < 5)
        <a href="{{ route('payment.create') }}" class="mt-6 w-full border-2 border-dashed border-gray-300 rounded-xl py-3 flex items-center justify-center gap-2 text-gray-500 font-bold text-sm hover:border-blue-500 hover:text-blue-500 transition-colors bg-gray-50/50">
